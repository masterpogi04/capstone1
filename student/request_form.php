...
if ($pending_count > 0) {
    ...
    $stmt = $connection->prepare("
        SELECT s.first_name, s.middle_name, s.last_name, s.student_id, s.gender, s.section_id, s.email,
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($stmt->execute()) {
            // Format the student name properly with middle initial if available
            $proper_name = ucwords(strtolower($student['first_name']));
            if (!empty($student['middle_name'])) {
                $middle_initial = strtoupper(substr($student['middle_name'], 0, 1)) . '.';
                $proper_name .= ' ' . $middle_initial;
            }
            $proper_name .= ' ' . ucwords(strtolower($student['last_name']));
            ...
            $notification_message = "New document request from {$proper_name} for {$documentRequest}";
            $notification_link = "facilitator_requested_documents.php?request_id=" . $request_id;
            
            $notify_facilitators->bind_param("ss", $notification_message, $notification_link);
            $notify_facilitators->execute();
...
<head>
    ...
    <style>
/* Base styles - already good but enhancing for better responsiveness */
body {
