...
$stmt = $connection->prepare("SELECT name FROM $table_name WHERE id = ?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $reportedByType = $_SESSION['user_type']; // Get the user type from the session
...
function attachStudentIdListener(input) {
    $(input).on('blur', function() {
        ...
        $.ajax({
            url: 'get_student_info-adviser.php',
            method: 'POST',
...
$(document).ready(function() {
    ...
    $('#incidentPlace, #incidentDate, #incidentTime').on('change', updateCombinedField);
