...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
       ...
       // Insert notifications for all facilitators except the current one if the reporter is a facilitator
        if ($reporter_type === 'facilitator') {
            // Notification for other facilitators
            $notify_other_facilitators = $connection->prepare("INSERT INTO notifications (user_type, user_id, message, link) 
                                                          SELECT 'facilitator', id, ?, ? 
                                                          FROM tbl_facilitator 
                                                          WHERE status = 'active' AND id != ?");
            
            $notification_message = "New incident report submitted by Facilitator " . $reportedBy;
            $notification_link = "view_facilitator_incident_reports.php?id=" . $incident_report_id;
            
            $notify_other_facilitators->bind_param("ssi", $notification_message, $notification_link, $reporter_id);
            if (!$notify_other_facilitators->execute()) {
                throw new Exception("Error inserting facilitator notifications: " . $notify_other_facilitators->error);
            }
        } else {
            // If reporter is not a facilitator, notify all facilitators
            $notify_facilitators = $connection->prepare("INSERT INTO notifications (user_type, user_id, message, link) 
                                                      SELECT 'facilitator', id, ?, ? 
                                                      FROM tbl_facilitator 
                                                      WHERE status = 'active'");
            
            $notification_message = "New incident report submitted by " . ucfirst($reporter_type) . " " . $reportedBy;
            $notification_link = "view_facilitator_incident_reports.php?id=" . $incident_report_id;
