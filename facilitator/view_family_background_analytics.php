            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="form-check form-check-inline">
                        <input class="form-check-input chart-toggle" type="checkbox" id="incomeCheck" checked data-chart="incomeContainer">
                        <label class="form-check-label" for="incomeCheck">Family Income Distribution</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input chart-toggle" type="checkbox" id="parentStatusCheck" checked data-chart="parentStatusContainer">
                        <label class="form-check-label" for="parentStatusCheck">Parent Status Distribution</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input chart-toggle" type="checkbox" id="birthOrderCheck" checked data-chart="birthOrderContainer">
                        <label class="form-check-label" for="birthOrderCheck">Birth Order Distribution</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input chart-toggle" type="checkbox" id="siblingsCheck" checked data-chart="siblingsContainer">
...
function updateCharts() {
    ...
    $.ajax({
        ...
        success: function(data) {
            let noDataCount = 0;
            const totalCharts = [showIncome, showParentStatus, showBirthOrder, showSiblings].filter(Boolean).length;

            // Handle each chart with no data check
            const chartConfigs = [
                { 
                    show: showIncome, 
                    container: 'incomeContainer', 
                    chartId: 'incomeChart', 
                    dataKey: 'family_income', 
                    data: data.family_income, 
                    title: 'Family Income Distribution' 
                },
                { 
                    show: showParentStatus, 
                    container: 'parentStatusContainer', 
                    chartId: 'parentStatusChart', 
                    dataKey: 'parent_status', 
                    data: data.parent_status, 
                    title: 'Parent Status Distribution' 
                },
                { 
                    show: showBirthOrder, 
                    container: 'birthOrderContainer', 
                    chartId: 'birthOrderChart', 
                    dataKey: 'birth_order', 
                    data: data.birth_order, 
                    title: 'Birth Order Distribution' 
                },
                { 
                    show: showSiblings, 
                    container: 'siblingsContainer', 
                    chartId: 'siblingsChart', 
                    dataKey: 'siblings', 
                    data: data.siblings, 
                    title: 'Siblings Distribution' 
                }
            ];
            ...
            chartConfigs.forEach(config => {
                ...
                if (config.show) {
                    if (config.data && config.data.length > 0) {
                        // Recreate canvas element if it doesn't exist
                        if (chartContainer.find('canvas').length === 0) {
                            chartContainer.html(`<canvas id="${config.chartId}"></canvas>`);
                        }
                        createChart(config.chartId, config.data, config.dataKey, config.title);
                    } else {
                        noDataCount++;
                        chartContainer.html(`
                            <div class="text-center p-4">
                                <div class="text-muted">
                                    ...
                                    <p>No data available for ${config.title.toLowerCase()}</p>
                                </div>
                            </div>
                        `);
                    }
            ...
            // Show SweetAlert notification if all visible charts have no data
            if (noDataCount > 0 && noDataCount === totalCharts) {
                const departmentText = $('#departmentSelect option:selected').text();
                const courseText = $('#courseSelect option:selected').text();
                
                let filterInfo = '';
                if (departmentText !== 'All Departments') {
                    filterInfo = `for ${departmentText}`;
                    if (courseText !== 'All Courses') {
                        filterInfo += ` > ${courseText}`;
                    }
                }
            }
        },
        ...
        $(document).ready(function() {
            ...
            $('.chart-toggle').change(function() {
                const chartContainer = $(this).data('chart');
                $(`#${chartContainer}`).toggleClass('hidden', !$(this).prop('checked'));
            });
            ...
            $('#resetFilters').click(function() {
                ...
                $('.chart-toggle').prop('checked', true);
