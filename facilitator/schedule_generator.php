...
<body>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <select class="form-control" id="time_input" required>
                                ...
                                <?php
                                // Generate time slots from 8 AM to 4 PM with 1-hour intervals
 ...
 document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateTimeOptions() {
        ...
        const currentSelection = timeInput.value;
        ...
        for (let hour = 8; hour < 16; hour++) {
            ...
            timeObj.setHours(hour, 0, 0);
            
            // Format for display
            const displayTime = new Date(timeObj).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
            
            // If today, only show future times (add 15 min buffer)
            const bufferTime = new Date();
            bufferTime.setMinutes(bufferTime.getMinutes() + 15);
            
            if (!isToday || timeObj > bufferTime) {
        ...
        // If no options available after filtering
        if (timeInput.options.length === 1) {
            ...
            timeInput.value = currentSelection;
            if (timeInput.value !== currentSelection) {
                timeInput.selectedIndex = 0; // Reset if not available
            }
        }
    }
    ...
    document.getElementById('schedulingForm').addEventListener('submit', async function(e) {
        ...
        // Update time options to ensure they're current
        updateTimeOptions();
    ...
    <?php if($existing_meeting_date): ?>
        ...
        // Round to nearest hour for 1-hour intervals
        let hour = existingDate.getHours();
        let minute = existingDate.getMinutes();
        if (minute >= 30) hour = (hour + 1) % 24;
        
        timeInput.value = hour.toString().padStart(2, '0') + ':00';
        updateTimeOptions(); // Update time options after setting date
    <?php endif; ?>
