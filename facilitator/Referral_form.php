...
// Get counselor name
$stmt = $connection->prepare("SELECT first_name, middle_initial, last_name, id FROM tbl_counselor LIMIT 1");
$stmt->execute();
$result = $stmt->get_result();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt->execute()) {
        // Get the ID of the newly inserted referral
        $referral_id = $connection->insert_id;
        
        // Insert notification for counselor
        $counselor_notif_message = "A new referral form has been submitted to you, from CEIT Guidance Office by facilitator: " . $facilitator_name;
        $counselor_link = "view_referrals_page.php?id=" . $referral_id;
        
        $counselor_notif_sql = "INSERT INTO notifications (user_type, user_id, message, link) 
                              VALUES ('counselor', ?, ?, ?)";
        $stmt_counselor_notif = $connection->prepare($counselor_notif_sql);
        $stmt_counselor_notif->bind_param("sss", $counselor_id, $counselor_notif_message, $counselor_link);
        $stmt_counselor_notif->execute();
        
        // Insert notification for student
        $student_notif_message = "You have been referred to the guidance counselor office. Kindly wait for further details";
        $student_link = "view_student_referrals.php";
        
        $student_notif_sql = "INSERT INTO notifications (user_type, user_id, message, link) 
                            VALUES ('student', ?, ?, ?)";
        $stmt_student_notif = $connection->prepare($student_notif_sql);
        $stmt_student_notif->bind_param("sss", $student_id, $student_notif_message, $student_link);
        $stmt_student_notif->execute();
...
<body>
    ...
    <div class="container">
        ...
        <a href="guidanceservice.html" class="modern-back-button">
            ...
            <form method="POST" action="" id="referralForm">
                ...
                <div class="form-group">
                    <label class="form-label" for="student_id">Student Number:</label>
                    <input type="text" class="form-control" id="student_id" name="student_id" pattern="[0-9]{9}" maxlength="9" inputmode="numeric" title="Please enter a 9-digit number" required>
                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            // Allow only numbers in student_id field
            ...
            $('#student_id').on('blur', function() {
                ...
                if (studentId && studentId.length === 9) {
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                Swal.fire({
                                    ...
                                    text: 'Please check the student number.',
                                    icon: 'warning',
                    ...
                    Swal.fire({
                        ...
                        text: 'Student number must be 9 digits.',
                        icon: 'warning',
                        confirmButtonColor: '#3085d6'
                    });
                    $('#student_name, #course_year').val('');
