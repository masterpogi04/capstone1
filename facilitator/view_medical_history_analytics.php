...
// Modified createChart function to properly handle legends when no data is available
function createChart(elementId, data, key, title) {
    ...
    const legendId = `${elementId}Legend`;
    const legendWrapperId = `${elementId}LegendWrapper`;
    
    // Get existing legend elements or create new ones
    let legendWrapper = document.getElementById(legendWrapperId);
        ...
        chartContainer.innerHTML = `
            <div class="text-center p-4">
                <div class="text-muted">
                    ...
                    <p>No data available for ${title.replace(/_/g, ' ').toLowerCase()}</p>
                </div>
            </div>
        `;
        
        // Remove the legend wrapper if it exists
        if (legendWrapper) {
            legendWrapper.innerHTML = ''; // Clear the contents
            if (legendWrapper.parentNode) {
                legendWrapper.parentNode.removeChild(legendWrapper);
            }
        }
        
        // If there was an existing chart, destroy it
        if (charts[elementId]) {
    ...
    // Initialize legend elements if they don't exist
    if (!legendWrapper) {
        legendWrapper = document.createElement('div');
        legendWrapper.id = legendWrapperId;
...
// Modified applyFilters function to properly handle chart and legend display
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                chartConfigs.forEach(config => {
                    ...
                    if (isVisible) {
                        ...
                        // Handle the chart creation through the createChart function
                        // which will properly handle both the chart and legend
                        const container = $(config.containerId).find('.chart-container');
                        
                        // Remove any existing legend for this chart
                        const legendWrapperId = `${config.chartId}LegendWrapper`;
                        const legendWrapper = document.getElementById(legendWrapperId);
                        if (legendWrapper && (!chartData || chartData.length === 0)) {
                        ...
                        // Reset the chart container to have a canvas element
                        container.html(`<canvas id="${config.chartId}"></canvas>`);
                        
                        // Create chart - createChart will handle the "no data" case
                        createChart(
                            config.chartId, 
                            chartData, 
                            config.dataKey, 
            ...
            error: function(xhr, status, error) {
                ...
                $('.chart-container:not(.hidden)').each(function() {
                    ...
                    // Also clear any related legends
                    const chartId = $(this).find('canvas').attr('id');
                    if (chartId) {
                        const legendWrapperId = `${chartId}LegendWrapper`;
                        const legendWrapper = document.getElementById(legendWrapperId);
                        if (legendWrapper) {
                            legendWrapper.innerHTML = '';
                            if (legendWrapper.parentNode) {
        ...
        if (departmentId) {
            ...
            $.ajax({
                ...
                success: function(courses) {
                    ...
                    applyFilters(); // Automatically apply filters after loading courses
                },
                error: function() {
            ...
            applyFilters(); // Automatically apply filters when department is cleared
        }
    });

    // Course change handler
    $('#courseSelect').change(function() {
        applyFilters(); // Automatically apply filters when course changes
    });
