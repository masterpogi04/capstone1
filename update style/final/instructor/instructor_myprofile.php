...
$stmt = $connection->prepare("SELECT username, email, first_name, middle_initial, last_name, profile_picture FROM tbl_instructor WHERE id = ?");
if (!$stmt) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Update profile
    $update_stmt = $connection->prepare("UPDATE tbl_instructor SET username = ?, email = ?, first_name = ?, middle_initial = ?, last_name = ?, updated_at = NOW() WHERE id = ?");
    if (!$update_stmt) {
        die("Error preparing update statement: " . $connection->error);
    }
    $update_stmt->bind_param("sssssi", $new_username, $new_email, $new_first_name, $new_middle_initial, $new_last_name, $instructor_id);
