...
$stmt = $connection->prepare("SELECT username, email, first_name, middle_initial, last_name, profile_picture FROM tbl_facilitator WHERE id = ?");
if ($stmt === false) {
...
if ($result->num_rows === 1) {
    ...
    $middle_initial = $facilitator['middle_initial'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $update_stmt = $connection->prepare("UPDATE tbl_facilitator SET first_name = ?, middle_initial = ?, last_name = ?, updated_at = NOW() WHERE id = ?");
    $update_stmt->bind_param("sssi", $new_first_name, $new_middle_initial, $new_last_name, $facilitator_id);
