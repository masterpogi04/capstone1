...
// Ensure the user is logged in and is either a counselor or facilitator
if (!isset($_SESSION['user_id']) || ($_SESSION['user_type'] !== 'counselor' && $_SESSION['user_type'] !== 'facilitator')) {
    ...
    <body>
        <script>
            ...
            Swal.fire({
                ...
                html: "No student profile found or error accessing profile data.<br><br>This window will attempt to close in <b></b> seconds...",
                timer: 5000,
                ...
                window.close();
                // If the window does not close, show a message asking the user to close it manually
                setTimeout(() => {
                    Swal.fire({
                        icon: "info",
                        title: "Please Close This Tab",
                        text: "We could not close the tab automatically. Please close it manually.",
                        allowOutsideClick: false
                    });
                }, 1000);
            });
...
if (!empty($profile['signature_path'])) {
    $full_signature_path = $_SERVER['DOCUMENT_ROOT'] . $profile['signature_path'];
...
// Get the logo path with correct server path
$logo_path = $_SERVER['DOCUMENT_ROOT'] . '/capstone1/counselor/logo.png';
if (file_exists($logo_path)) {
    $logo_data = base64_encode(file_get_contents($logo_path));
} else {
    // Provide a fallback or error message if logo doesn't exist
...
$dompdf->addInfo("Author", "CEIT Guidance counselor");
