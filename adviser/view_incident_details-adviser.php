...
// Function to format the reporter's name
function formatReporterName($incident) {
    if (!isset($incident['reported_by_type']) || !isset($incident['reporters_id'])) {
        return 'Unknown';
    }

    $table_name = 'tbl_' . $incident['reported_by_type'];
    global $connection;
    ...
    $stmt = $connection->prepare($query);
    $stmt->bind_param("i", $incident['reporters_id']);
    $stmt->execute();
    $result = $stmt->get_result();
    $reporter = $result->fetch_assoc();
    
    if ($reporter) {
        $middle_initial = !empty($reporter['middle_initial']) ? " {$reporter['middle_initial']}. " : " ";
        return $reporter['first_name'] . $middle_initial . $reporter['last_name'];
    }
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <p><span class="label">Description:</span> <?php echo htmlspecialchars($incident['description']); ?>
                </p>
                <p>
