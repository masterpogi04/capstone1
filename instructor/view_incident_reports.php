                                ...
                                <td colspan="7" class="text-center">
                                    <?php if (!empty($search)): ?>
                                        <div class="alert alert-warning" role="alert">
                                            No incident reports found for "<strong><?php echo htmlspecialchars($search); ?></strong>".
                                        </div>
                                    <?php elseif (!empty($department) || !empty($course)): ?>
                                        <div class="alert alert-warning" role="alert">
                                            No incident reports found for the selected 
                                            <?php 
                                            echo !empty($department) ? "department" : "";
                                            echo (!empty($department) && !empty($course)) ? " and " : "";
...
// Real-time search with client-side filtering
    $("#searchInput").on('input', function() {
        ...
        // Always hide the "no data" row before filtering
        $("table tbody .no-data-row").remove();
        $("table tbody tr").each(function() {
            var searchData = $(this).text().toLowerCase();
            var match = searchData.includes(searchText);
            $(this).toggle(match);
        ...
        if (!anyVisible) {
            $("table tbody").append(
                '<tr class="no-data-row"><td colspan="7" class="text-center"><div class="alert alert-warning" role="alert">No incident reports found for "<strong>' +
                $('<div>').text($("#searchInput").val()).html() +
                '</strong>".</div></td></tr>'
            );
        }
    });
